...
def calcula_media_duracao_notas(stream_obj):
    ...
    return duracao/len(stream_obj.flatten().notes)
...
# Exemplo de processamento de um arquivo midi
def processa_midi(file_path):
    ...
    # Distribuição da duração das notas
    bins = [0, 0.5, 1, 2, 4, float('inf')]
    labels = ['<0.5', '0.5-1', '1-2', '2-4', '>4']
    duracoes = [n.quarterLength for n in base_midi.flatten().notes]
    hist, _ = np.histogram(duracoes, bins=bins)
    for i, label in enumerate(labels):
        col = f'NOTA_DURACAO_{label}'
        data.setdefault(col, []).append(hist[i])
    # Distribuição da duração das pausas
    pausas = [r.quarterLength for r in base_midi.flatten().getElementsByClass('Rest')]
    hist_pausa, _ = np.histogram(pausas, bins=bins)
    for i, label in enumerate(labels):
        col = f'PAUSA_DURACAO_{label}'
        data.setdefault(col, []).append(hist_pausa[i])
    # Nome do arquivo como compositor (ou extraia do nome do arquivo)
    data['COMPOSITOR'].append('Ludwig van Beethoven')
# Exemplo de uso para um arquivo:
# processa_midi('/caminho/para/arquivo.mid')

# Para processar todos os arquivos midi de um diretório:
...
# Exemplo de uso:
# processa_diretorio_midi('/caminho/para/seus/midis')

processa_diretorio_midi('/home/positivetoad/projetos/ML/musica/projeto_TA/midi/beeth2')  # Substitua pelo caminho do diretório com arquivos MIDI
# Crie o DataFrame novo normalmente
df_novo = pd.DataFrame(data)

# Se o arquivo já existe, leia e concatene
